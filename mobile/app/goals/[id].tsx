...
export default function GoalDetailScreen() {
  ...
  const loadGoal = useCallback(async () => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail || err.message || 'Failed to load goal details.';
      setError(detail);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleAddProgress = async () => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail || err.message || 'Unable to add progress update.';
        ...
        { text: 'Not yet', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const detail = err.response?.data?.detail || err.message || 'Failed to update goal status.';
              Alert.alert('Update failed', detail);
            } finally {
  ...
  return (
    <>
      ...
      >
        <PageHeading
          title={goal.title}
          description={goal.description || 'Keep momentum with focused actions and shared reflections.'}
          actions={
            goal.status !== 'completed' ? (
              <TouchableOpacity
                style={[styles.primaryAction, markingComplete && styles.primaryActionDisabled]}
        ...
        >
          <View style={styles.heroRow}>
            ...
            <View style={styles.heroBadge}>
              <Text style={styles.heroBadgeText}>{goal.status.toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.heroDates}>
            <View style={styles.heroCol}>
              <Text style={styles.heroLabel}>Target date</Text>
              <Text style={styles.heroValue}>{formatDate(goal.target_date)}</Text>
            </View>
            <View style={styles.heroCol}>
              <Text style={styles.heroLabel}>Created</Text>
              <Text style={styles.heroValue}>{formatDate(goal.created_at)}</Text>
            </View>
            {goal.completed_at && (
              <View style={styles.heroCol}>
                <Text style={styles.heroLabel}>Completed</Text>
                <Text style={styles.heroValue}>{formatDate(goal.completed_at)}</Text>
              </View>
            )}
          </View>
        </LinearGradient>
          ...
          <Card style={styles.momentumCard}>
            <View style={styles.momentumRow}>
              <View style={styles.momentumStat}>
                <Text style={styles.momentumValue}>{goal.progress_updates ?? goal.progress?.length ?? 0}</Text>
                <Text style={styles.momentumLabel}>Updates logged</Text>
              </View>
              <View style={styles.momentumDivider} />
              <View style={styles.momentumStat}>
                <Text style={styles.momentumValue}>{goal.status === 'completed' ? '100%' : 'In flight'}</Text>
        ...
        >
          <Card style={styles.addProgressCard}>
            ...
            />
            <TouchableOpacity
              style={[styles.secondaryAction, addingProgress && styles.secondaryActionDisabled]}
        ...
        <Section
          title="Timeline"
          subtitle={goal.progress && goal.progress.length > 0 ? 'Snapshots of your journey together.' : 'No updates yet — your first reflection will appear here.'}
        >
          {goal.progress && goal.progress.length > 0 ? (
                ...
                .map((item, index) => (
                  <View key={`${item.date}-${index}`} style={styles.timelineItem}>
                    <View style={styles.timelineMarker}>
                      ...
                      {index < goal.progress!.length - 1 && <View style={styles.timelineLine} />}
                    </View>
                    <Card style={styles.timelineCard}>
                      ...
                      {typeof item.progress_value === 'number' && (
                        <Text style={styles.timelineValue}>Progress score: {(item.progress_value * 100).toFixed(0)}%</Text>
                      )}
                    </Card>
                  </View>
                ))}
            ...
            <Card>
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyBody}>Add a reflection to see your momentum build over time.</Text>
              </View>
            </Card>
          )}
        </Section>
        ...
        >
          <Card style={styles.supportCard}>
            <Ionicons name="chatbubbles-outline" size={20} color={colors.brand[500]} />
            <Text style={styles.supportText}>Email hello@heka.app for guidance or to celebrate your wins.</Text>
          </Card>
        </Section>
      </ScrollView>
    </>
